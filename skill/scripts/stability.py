...
def load_baseline():
    if not os.path.exists(BASELINE):
        return {}
    rows = list(csv.DictReader(open(BASELINE, encoding="utf-8-sig", newline="")))
    out = {}
    for r in rows:
...
def main() :
    ...
    def pick_best_candidate(exact, starts_with, contains, similar):
        """search_company의 4개 그룹에서 1순위 후보를 하나로 picks. 없으면 None."""
    ...
    def print_diagnosis(r, out, base_src_note=None, max_total_move=None):
        ...
        # ── A-1. 순증감 대 총이동 막대 (표 아래 블록, 두 막대 동일 축척) ─────────────
        ...
        # ── A-2. 업종 대비 막대 (표 아래 블록, 두 막대 동일 40칸 축척) ──────────────
        b = base.get(r["업종"], allmed) or allmed
        ...
        # ── C-3. 업종 내 위치 (분위수). 기준선에 분위수 컬럼이 있으면 출력, 없으면 이 줄만 건너뛴다.
    ...
    if compare_targets:
        ...
        for t in compare_targets:
            exact, starts_with, contains, similar = search_company(ROWS, t)
            best = pick_best_candidate(exact, starts_with, contains, similar)
            if best is None:
                skipped.append(t)
            else:
        ...
        if not found_rows:
            ...
            if len(inds) > 1:
                ...
                P("")
            cols = [r["사업장명"] for r in found_rows]
            hdr = ["지표"] + cols
            P("| " + " | ".join(hdr) + " |")
            P("| " + " | ".join(["---"] * len(hdr)) + " |")
            def _r(label, fn):
                cells = [label] + [fn(r) for r in found_rows]
                P("| " + " | ".join(cells) + " |")
            ...
            _r("추정소득", _est)
            P("")
            P("**비교 참고** — 이 표의 업종배수 기준은 동봉 기준선(2026-07 전국 52,957개소, %s개 업종)입니다." % "{:,}".format(len(bundled)))
            P("")
            ...
            if not exact and not starts_with and not contains and not similar:
                ...
                def group_label(lst):
                    ...
                    if lst is similar:
                        return "유사명"
                    return None
                groups_info = [(group_label(lst), lst) for lst in (exact, starts_with, contains, similar)]
                shown_groups = [(g, lst) for g, lst in groups_info if lst]

                if len(cand) == 1:
                    P("→ 후보가 1건뿐이므로 바로 진단합니다.")
                    P("")
                    print_diagnosis(cand[0], P)
                elif pick is not None:
                    try:
                        pick_i = int(pick) - 1
                    except (TypeError, ValueError):
                        P("'%s'는 번호로 해석할 수 없습니다. 1~%d 사이의 번호를 달라."
                        % (pick, len(cand)))
                        P("")
                    ...
                    if not (0 <= pick_i < len(cand)):
                        P("'%s'는 1~%d 범위를 벗어납니다. 다시 검색하거나 번호를 확인하세요."
                        % (pick, len(cand)))
                        P("")
                        ...
                        sys.exit(0)
                    P("→ %d건 중 %d번을 진단합니다 - %s"
                    % (len(cand), pick_i + 1, cand[pick_i]["사업장명"]))
                    P("")
                    print_diagnosis(cand[pick_i], P)
                else:
                    for g, lst in shown_groups:
                        P("- [%s] (%d건)" % (g, len(lst)))
                        for r in lst[:10]:
                    ...
                    P("")
                    P("더 정확한 이름으로 다시 검색하시거나, 번호를 알려주시면 해당 사업장을 진단합니다. (예: --company 쿠팡풀필먼트 --pick 2)")
                    P("")
        else:
            # ── 전체 리포트 ───────────────────────────────────────────────
            N = len(analyzed)
