export default function HiringInsight({
  ...
  const 충원 = sorted.reduce((s, r) => s + Math.min(r.신규, r.상실), 0);
  const 증원 = sorted.reduce((s, r) => s + Math.max(r.신규 - r.상실, 0), 0);
  const 줄어든 = sorted.reduce((s, r) => s + Math.max(r.상실 - r.신규, 0), 0);
  ...
  // 몰린달
  const 몰린달목록 = sorted
    .filter((r) => r.신규 >= 평균신규 * 1.5 && r.신규 >= 3)
    .sort((a, b) => b.신규 - a.신규)
    .slice(0, 3)
    .map((r) => ({
      자료년월: r.자료년월,
      신규: r.신규,
      배수: Math.round((r.신규 / 평균신규) * 10) / 10,
    }));

  const 몰린달칩들 = 몰린달목록.map((m) => (
    <span
      key={m.자료년월}
  ...
  let 몰린달영역: React.ReactNode;
  if (몰린달목록.length > 0) {
    몰린달영역 = <div className="mt-1 flex flex-wrap gap-2">{몰린달칩들}</div>;
  } else if (신규합 > 0) {
    몰린달영역 = (
  ...
  const 변화꼬리 = (변화퍼: number | null, 이전: number) =>
    변화퍼 !== null
      ? `, ${변화퍼 > 0 ? "+" : ""}${변화퍼}%`
      : "";
  ...
  const 신규칩 = (
    <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-800">
      ...
      {Math.round(이전신규).toLocaleString("ko-KR")}명{변화꼬리(신규변화퍼, 이전신규)})
    </span>
  );
  const 상실칩 = (
    <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs text-zinc-800">
      상실 월평균 {Math.round(최근상실).toLocaleString("ko-KR")}명 (그 전{" "}
      {Math.round(이전상실).toLocaleString("ko-KR")}명{변화꼬리(상실변화퍼, 이전상실)})
    </span>
  );
  ...
  return (
    <section className="mt-3 rounded-lg border border-zinc-200 bg-white p-4">
      ...
      <div className="mt-1 flex flex-wrap gap-x-3 text-xs text-zinc-600">
        <span className="text-zinc-700">■ 빈자리 채우기 {strings.충원}명 ({strings.충원퍼})</span>
        <span className="text-emerald-500">■ 늘어난 자리 {strings.증원}명 ({strings.증원퍼})</span>
      </div>
      <p className="mt-1 text-xs text-zinc-500">
        같은 기간 신규취득은 평균 직원 수의 {strings.신규비율}예요{줄어든접미사}.
      </p>
      {팁}
      ...
      <p className="mt-4 text-xs text-zinc-500">
        같은 달 안에서 나간 만큼 들어온 인원을 ‘빈자리 채우기’, 그보다 더 들어온 인원을 ‘늘어난 자리’로 셌어요.
        신규취득·상실은 입사·퇴사와 같지 않아요(전보·재가입 포함).
      </p>
    </section>
  );
}
