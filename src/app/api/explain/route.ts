...
4. 이번 달은 사실 목록의 '이번 달 인원 흐름' 분류에 맞춰서만 설명한다.
   - "안에서 드나든 인원이 많은 달"일 때만 "전체 인원 변화는 작지만 안에서는 신규취득과 상실이 많이 일어났어요"처럼 말한다.
   - "인원이 늘어난 달"이면 늘어난 달로, "인원이 줄어든 달"이면 줄어든 달로 말한다. 이때 "거의 그대로", "변화는 작지만"이라고 쓰지 않는다.
...
8. 사실 목록에 '채용의 성격'이 있으면, 이 회사의 신규취득이 빈자리를 채우는 쪽인지 인원을 늘리는 쪽인지, 신규취득이 몰리는 달이 있는지, 최근 3개월에 달라졌는지를 지원자 관점에서 짚는다.
   - 지원_전에_확인해보세요 에 이 흐름에 맞는 확인 거리를 한 개 이상 넣는다. 채용의 성격은 이미 계산돼 있으니 "증원인지, 빈자리를 채우는 채용인지"를 되묻지 말고 결과에 맞춰 묻는다. 예: 모두 또는 대부분 빈자리 채우기면 "지원하는 자리가 비게 된 이유와 인수인계 방식", 늘어난 자리가 많으면 "인원이 늘어나는 팀과 직무", 몰린 달이 있으면 "채용이 정해진 시기에 몰리는지", 최근 신규취득이 크게 줄었으면 "최근 채용 규모가 줄어든 배경".
   - '몇 개월 간격으로 몰림'이 있으면 그 간격을 그대로 말한다. '최근 신규취득이 크게 줄었음/늘었음', '최근 상실이 크게 늘었음'이 있으면 그 흐름을 말한다.
   - 원인을 지어내지 않는다. 쓰지 말 것: "구조조정", "해고", "정년", "공채 때문", "사업이 어려워", "성장하고 있어요".

항목별 형식 (개수를 꼭 지킨다)
- 지원자_관점_요약: 한 문장, 50자 이내. 지원자가 가장 먼저 알아야 할 흐름 한 가지.
  예(12개월 동안 늘고 업종 안에서 높은 편일 때): "직원 수는 늘고 있지만, 같은 업종보다 인력 이동은 빠른 편이에요."
  예(안에서 드나든 인원이 많은 달일 때만): "전체 인원은 거의 그대로인데 실제로는 많은 인력이 오간 사업장이에요."
- 이렇게_볼_수_있어요: 2~3문장. 첫 문장은 이번 달 인원 흐름, 둘째 문장은 12개월 흐름(연속·일시적·최근 변화)을 말한다. 채용의 성격은 여기에 쓰지 않고 채용_흐름 에 쓴다.
...
- 지원_전에_확인해보세요: 2~3개. 지원자가 채용 공고나 면접에서 확인할 거리를 짧은 명사형으로 쓴다. 사실 목록의 흐름에 맞는 것을 고른다.
  인력 이동이 큰 경우 예: "최근 조직 개편이나 대규모 채용이 있었는지", "지원 직무 팀의 구성과 근속 현황"
  가입자 수가 늘어나는 경우 예: "신규 사업이나 조직 확대 여부", "새로 합류한 사람을 위한 온보딩 체계"
  '채용의 성격'이 없을 때만 예: "이번 채용이 증원인지, 빈자리를 채우는 채용인지"
- 채용_흐름: 사실 목록에 '채용의 성격'이 있으면 한 문장, 70자 안팎. 빈자리 채우기와 늘어난 자리 중 어느 쪽인지 사실 목록의 숫자로 말하고, 몰린 달·몇 개월 간격·최근 3개월 신규취득 변화 중 가장 두드러진 것 하나를 이어 붙인다. '채용의 성격'이 없으면 빈 문자열.
...
function flowType(d: any): { 흐름: string; 설명: string } {
  ...
  if (순증감 === 0) {
    return { 흐름: "드나듦", 설명: "안에서 드나든 인원이 많은 달 (총인원 변화에 비해 신규취득·상실이 훨씬 많음)" };
  }
  ...
  if (순증감 > 0) {
    return { 흐름: "증가", 설명: "인원이 늘어난 달 (드나든 사람 대부분이 늘어난 인원)" };
  }
  return { 흐름: "감소", 설명: "인원이 줄어든 달 (드나든 사람 대부분이 줄어든 인원)" };
}
...
function buildFactList(
  ...
  if (추이 && 추이.length >= 6) {
    ...
    // 10a — 12개월 채용 흐름
    const 신규합 = sorted.reduce((s, it) => s + it.신규, 0);
    const 상실합 = sorted.reduce((s, it) => s + it.상실, 0);
    const 충원 = Math.min(신규합, 상실합);
    const 증원 = Math.max(신규합 - 상실합, 0);
    const 줄어든 = Math.max(상실합 - 신규합, 0);
    const 평균인원 = sorted.reduce((s, it) => s + it.가입자수, 0) / n;
    const 신규비율 = Math.round(신규합 / 평균인원 * 100);
    const 충원퍼 = 신규합 > 0 ? Math.round(충원 / 신규합 * 100) : 0;
    const 증원퍼 = 100 - 충원퍼;
    const 평균신규 = 신규합 / n;

    // 채용의 성격
    if (신규합 === 0) {
      f.push(`최근 ${n}개월 채용의 성격: 신규취득 없음, 같은 기간 상실 ${fmt(상실합)}명`);
    } else if (증원 === 0) {
      let 성격 = `최근 ${n}개월 채용의 성격: 신규취득 ${fmt(신규합)}명은 모두 나간 인원만큼 다시 채운 규모(빈자리 채우기), 같은 기간 상실 ${fmt(상실합)}명`;
      if (줄어든 > 0) 성격 += `, 채우지 않고 줄어든 자리 ${fmt(줄어든)}명`;
      f.push(성격);
    } else {
      f.push(`최근 ${n}개월 채용의 성격: 신규취득 ${fmt(신규합)}명 중 빈자리 채우기 ${fmt(충원)}명(${충원퍼}%), 늘어난 자리 ${fmt(증원)}명(${증원퍼}%), 같은 기간 상실 ${fmt(상실합)}명`);
    }

    // 신규합 > 0 일 때만
    if (신규합 > 0) {
      f.push(`최근 ${n}개월 신규취득 규모: 평균 가입자 수의 ${신규비율}%`);

      // 채용이 몰린 달
      const 후보목록 = sorted.filter(it => it.신규 >= 평균신규 * 1.5 && it.신규 >= 3);
      let 몰린달: any[] = [];
      if (후보목록.length > 0) {
        const sortedBy신규 = [...후보목록].sort((a, b) => b.신규 - a.신규);
        const top4 = sortedBy신규.slice(0, 4);
        몰린달 = top4.sort((a, b) => a.자료년월.localeCompare(b.자료년월));
      }

      if (몰린달.length === 0) {
        f.push(`채용이 몰린 달: 없음 (신규취득이 평소의 1.5배 이상인 달 없음)`);
      } else {
        const parts = 몰린달.map(it => {
          const 배수 = Math.round(it.신규 / 평균신규 * 10) / 10;
          return `${it.자료년월} ${fmt(it.신규)}명(평소의 ${배수}배)`;
        });
        let 몰린달문장 = `채용이 몰린 달(신규취득이 평소의 1.5배 이상): ` + parts.join(", ");
        // 간격
        let 간격: number | null = null;
        if (후보목록.length === 3 || 후보목록.length === 4) {
          const gaps: number[] = [];
          for (let i = 1; i < 몰린달.length; i++) {
            const y1m1 = 몰린달[i - 1].자료년월.split("-").map(Number);
            const y2m2 = 몰린달[i].자료년월.split("-").map(Number);
            gaps.push(y2m2[0] * 12 + y2m2[1] - (y1m1[0] * 12 + y1m1[1]));
          }
          if (gaps.length > 0 && gaps.every(g => g === gaps[0] && g >= 2)) {
            간격 = gaps[0];
          }
        }
        if (간격 !== null) {
        ...
        // 1월·7월 체크
        const hasJanJul = 몰린달.some(it => {
          const m = it.자료년월.split("-")[1];
          return m === "01" || m === "07";
        });
        if (hasJanJul) {
          몰린달문장 += ` · 1월·7월은 정기 인사이동이 섞일 수 있음`;
        }
        f.push(몰린달문장);
      }

      // 최근 3개월 vs 이전
      const 최근신규 = sorted.slice(-3).reduce((s, it) => s + it.신규, 0) / 3;
      const 최근상실 = sorted.slice(-3).reduce((s, it) => s + it.상실, 0) / 3;
      const 이전신규 = sorted.slice(0, n - 3).reduce((s, it) => s + it.신규, 0) / (n - 3);
      const 이전상실 = sorted.slice(0, n - 3).reduce((s, it) => s + it.상실, 0) / (n - 3);
      const 신규변화퍼 = 이전신규 > 0 ? Math.round((최근신규 - 이전신규) / 이전신규 * 100) : null;
      const 상실변화퍼 = 이전상실 > 0 ? Math.round((최근상실 - 이전상실) / 이전상실 * 100) : null;
      const 꼬리 = (변화퍼: number | null) => 변화퍼 !== null ? `, ${변화퍼 > 0 ? "+" : ""}${변화퍼}%` : "";

      let 변화문장 = `최근 3개월 월평균 신규취득 ${fmt(Math.round(최근신규))}명(그 전 ${n - 3}개월 ${fmt(Math.round(이전신규))}명${꼬리(신규변화퍼)}), 상실 ${fmt(Math.round(최근상실))}명(그 전 ${fmt(Math.round(이전상실))}명${꼬리(상실변화퍼)})`;
      const 신호: string[] = [];
      if (신규변화퍼 !== null && 신규변화퍼 <= -30 && 이전신규 >= 3) 신호.push("최근 신규취득이 크게 줄었음");
      if (신규변화퍼 !== null && 신규변화퍼 >= 30 && 이전신규 >= 3) 신호.push("최근 신규취득이 크게 늘었음");
      if (상실변화퍼 !== null && 상실변화퍼 >= 30 && 이전상실 >= 3) 신호.push("최근 상실이 크게 늘었음");
      if (신호.length > 0) 변화문장 += " · " + 신호.join(" · ");
      f.push(변화문장);
    }
  }
    ...
    response_format: {
      ...
      json_schema: {
        ...
        schema: {
          ...
          required: ["지원자_관점_요약", "이렇게_볼_수_있어요", "같은_업종과_비교하면", "지원_전에_확인해보세요", "판단_과정", "채용_흐름"],
...
function filterResult(
  ...
  let 이렇게 = raw이렇게.filter((s: string) => passNum(s, allowed) && passForbid(s)).slice(0, 3);
  ...
  // 채용_흐름 한 문장을 본문 셋째 문장으로 (사실 목록에 채용의 성격이 있고 검사를 통과할 때만)
  const 채용흐름 = typeof result.채용_흐름 === "string" ? result.채용_흐름.trim() : "";
  if (채용흐름 && factList.some((f: string) => f.includes("채용의 성격"))) {
    if (passNum(채용흐름, allowed) && passForbid(채용흐름)) {
      이렇게 = [...이렇게.slice(0, 2), 채용흐름];
    } else {
      removed++;
    }
  }
